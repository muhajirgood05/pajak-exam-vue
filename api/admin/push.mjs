...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!packages.length) {
      res.status(500).json({ ok: false, error: 'Paket soal tidak ditemukan.' });
      return;
    }
    ...
    if (!selected) {
      res.status(400).json({ ok: false, error: 'targetPackageId tidak ditemukan.' });
      return;
    }
    ...
    if (availableSlots <= 0) {
      res.status(400).json({ ok: false, error: `Paket ${selected.id} untuk ${targetSession} sudah penuh.` });
      return;
    }

    const toAppend = generatedQuestions.slice(0, availableSlots);
    const nextIdStart = currentList.reduce((max, item) => Math.max(max, Number(item?.id) || 0), 0) + 1;
